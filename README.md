...
Previously, products required the caller to supply an `id` in the request body. Sending a request without one would insert a document with an empty-string ID, and a second such request would collide. Categories already generated IDs on the backend.

**Fix:** `MongoProductRepository.Create` now generates a MongoDB ObjectID hex string when `product.ID` is empty, matching the existing behaviour in `MongoCategoryRepository`. Do **not** include an `id` field when creating products — the backend always generates one.
...
**CSV format** (header row required, 4 columns — no ID column):
...
- Rows with fewer than 4 columns or non-numeric `price`/`quantity` are silently skipped.
- Up to **10 rows are processed concurrently** via a worker-pool semaphore.
- Per-row errors are collected and returned — nothing is silently dropped.
...
`index` is the 0-based position of the failed row in the submitted product list (after filtering malformed rows). The response is always 207 so callers can inspect both `created` and `errors` regardless of outcome.
